...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const cleanUsername = username.trim().slice(0, 255);
    const cleanPassword = password.slice(0, 255);
    ...
    try {
      ...
      // Safe Open-Redirect Protection: Only allow internal paths starting with single '/'
      let targetUrl = "/";
  ...
  const publicWebUrl =
    process.env.NEXT_PUBLIC_WEB_URL || "https://standupindotimika.com";

  return (
    <div className="min-h-screen bg-[#FDFBF7] flex items-center justify-center p-6 md:p-12 selection:bg-[#FF4500] selection:text-white relative">
      {/* Back to Public Web Link */}
      <div className="absolute top-6 left-6 md:top-8 md:left-12 z-20">
        <a
          href={publicWebUrl}
          className="inline-flex items-center gap-2 px-4 py-2 bg-white border-2 border-black font-['Space_Mono',monospace] text-xs font-bold uppercase tracking-wider shadow-[4px_4px_0px_0px_#000000] hover:bg-[#FFE9E3] active:translate-x-[2px] active:translate-y-[2px] active:shadow-[1px_1px_0px_0px_#000000] transition-all cursor-pointer"
        >
      ...
      {/* Main Split Container with Smooth Zoom-In Spring Entrance */}
      <AnimateReveal
        variant="zoom-in"
        durationMs={700}
        className="w-full max-w-4xl my-12"
      >
        <div className="w-full bg-white border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] grid grid-cols-1 md:grid-cols-2 overflow-hidden">
